...
configure_emulator_locale() {
    ...
    "$adb_path" -s "$device_serial" shell "cmd locale set-app-locales com.example.meshtasticwear --locales $locale_tag" 2>/dev/null || true
    "$adb_path" -s "$device_serial" shell "cmd locale set-app-locales com.elishaazaria.sayboard --locales $locale_tag" 2>/dev/null || true
